...
class data_generator():
    ...
    def get_patch_pair(self,v_id,batch_count):
        ...
        # print("real min: ", np.min(real_img))
        # print("real max: ", np.max(real_img))
        ...
        img_augmented = (real_img - np.min(real_img)) / (np.max(real_img) - np.min(real_img))
        #img_augmented = real_img
        ...
        if (batch_count%2==0): #no gray background, 1-stage with prob
            ...
            # hier ist zumindest noch der hintergrund drauf, nur die objekte sind teilweise noch bisschen komisch gecroppt
            # probiere jetzt mit aug = 0
            background_mask = np.invert(mask_temp)
            #image_no_mask_zero[background_mask]=[0.5,0.5,0.5]

            # print("min before: ", np.min(image_no_mask_zero))
            # print("max before: ", np.max(image_no_mask_zero))
            image_no_mask_zero = self.seq_syn.augment_image(image_no_mask_zero*255)/255
            image_no_mask_zero = ( (image_no_mask_zero - np.min(image_no_mask_zero)) / (np.max(image_no_mask_zero) - np.min(image_no_mask_zero)) ) #* 2 - 1
            # print("min: ", np.min(image_no_mask_zero))
            # print("max: ", np.max(image_no_mask_zero))
            # plt.imshow((image_no_mask_zero+1)/2)
            # #plt.imshow((image_no_mask_zero+1)/2)
            # plt.title('image_no_mask_zero')
            # plt.show()
        ...
        # print("base_image:", np.min(base_image), np.max(base_image))
        # print("tgt_image:", np.min(tgt_image), np.max(tgt_image))
        # print("mask_area_crop:", np.min(mask_area_crop), np.max(mask_area_crop))
        # print()
        ...
        mask_area_resized = resize(mask_image.astype(np.float),(self.imsize,self.imsize),order=1,mode='reflect')
        ...
        # plt.imshow((image_no_mask_zero+1)/2)
        # plt.title('src_image_resized')
        # plt.show()
        # plt.imshow((tgt_image_resized+1)/2)
        # plt.title('ground_truth_resized')
        # plt.show()
        # plt.imshow((mask_area_resized+1)/2)
        # plt.title('mask_area_resized')
        # plt.show()
