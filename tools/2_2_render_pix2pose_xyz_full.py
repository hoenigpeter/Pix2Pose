...
for m_id,model_id in enumerate(model_ids):
        ...
        for img_id in range(len(rgb_files)):
            ...
            for gt_instance, gt in enumerate(gt_img):            
                ...
                # if visibility_percentage_bbox > 0.1:
                        #xyz_fn = os.path.join(xyz_dir,scene_string+"_"+img_string+".npy")
                xyz_sub_fn = os.path.join(xyz_sub_dir, f"{scene_string}_{img_string}_{gt_instance:06d}.jpg")
                rgb_sub_fn = os.path.join(rgb_sub_dir, f"{scene_string}_{img_string}_{gt_instance:06d}.jpg")

                cv2.imwrite(xyz_sub_fn, img_r[:, :, ::-1])
                cv2.imwrite(rgb_sub_fn, img[:, :, ::-1])
